...
DIST_ORIGIN_REQUEST_LAMBDA_ZIP_PATH="$DIR/../dist/origin-request/$FILENAME_LAMBDA_ZIP"
DIST_VIEWER_REQUEST_LAMBDA_ZIP_PATH="$DIR/../dist/viewer-request/$FILENAME_LAMBDA_ZIP"
DIST_CUSTOM_RDS_LAMBDA_ZIP_PATH="$DIR/../dist/custom-rds/$FILENAME_LAMBDA_ZIP"

STACK_NAME="serverless-allthethings-$BRANCH_SLUG"

cp $SRC_BRANCH_YAML_PATH $DIST_BRANCH_YAML_PATH

LAMBDA_APP_SHA1SUM="$(sha1sum $DIST_APP_LAMBDA_ZIP_PATH | sed 's/ .*$//')"
LAMBDA_ORIGIN_REQUEST_SHA1SUM="$(sha1sum $DIST_ORIGIN_REQUEST_LAMBDA_ZIP_PATH | sed 's/ .*$//')"
LAMBDA_VIEWER_REQUEST_SHA1SUM="$(sha1sum $DIST_VIEWER_REQUEST_LAMBDA_ZIP_PATH | sed 's/ .*$//')"
LAMBDA_CUSTOM_RDS_SHA1SUM="$(sha1sum $DIST_CUSTOM_RDS_LAMBDA_ZIP_PATH | sed 's/ .*$//')"

sed -i "s/LambdaVersionApp/LambdaVersionApp$LAMBDA_APP_SHA1SUM/g" "$DIST_BRANCH_YAML_PATH"
sed -i "s/LambdaVersionOriginRequest/LambdaVersionOriginRequest$LAMBDA_ORIGIN_REQUEST_SHA1SUM/g" "$DIST_BRANCH_YAML_PATH"
sed -i "s/LambdaVersionViewerRequest/LambdaVersionViewerRequest$LAMBDA_VIEWER_REQUEST_SHA1SUM/g" "$DIST_BRANCH_YAML_PATH"
sed -i "s/LambdaVersionCustomEnableHttpEndpoint/LambdaVersionCustomEnableHttpEndpoint$LAMBDA_CUSTOM_RDS_SHA1SUM/g" "$DIST_BRANCH_YAML_PATH"
